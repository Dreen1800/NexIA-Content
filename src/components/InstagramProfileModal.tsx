...
import { Instagram, Search, Loader, X, RefreshCw, Check, AlertCircle, Clock, Info, UserPlus, ExternalLink, Settings, Upload, Activity, TrendingUp, Users, Shield, CheckCircle, AlertTriangle } from 'lucide-react';
...
import ProfileHealthIndicator from './ProfileHealthIndicator';
...
export default function InstagramProfileModal({ isOpen, onClose }: InstagramProfileModalProps) {
    ...
    // Mostrar preview quando houver um perfil atual e posts
    useEffect(() => {
    ...
    const handleUpdateJob = async (jobId: string) => {
        try {
            ...
            setError(`Erro ao atualizar job: ${error.message}`);
        }
    };
    ...
    return (
        ...
        >
            <div
                className={`relative bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl transition-all duration-300 transform ${isModalVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'
                    }`}
            ...
            >
                ...
                <div className="flex">
                    ...
                    <div className={`${showProfilePreview ? 'w-1/2' : 'w-full'} p-6 transition-all duration-500`}>
                                                            ...
                                                            <li key={job.id} className="py-3">
                                                                <div className="flex items-center justify-between">
                                                                    ...
                                                                    <div className="flex items-center space-x-2">
                                                                        ...
                                                                        {(job.status === 'COMPLETED' || job.status === 'APIFY_SUCCEEDED') && job.apify_dataset_id && (
                                                                            <button
                                                                                onClick={() => handleUpdateJob(job.id)}
                                                                                className="inline-flex items-center px-2 py-1 text-xs font-medium text-[#9e46d3] bg-[#9e46d3]/10 hover:bg-[#9e46d3]/20 rounded-md transition-colors duration-200"
                                                                                title="Buscar dados completos e atualizar posts"
                                                                            >
                    ...
                    {/* Coluna direita - Preview da Saúde do Perfil */}
                    {showProfilePreview && currentProfile && (
                        <div className="w-1/2 border-l border-gray-200 p-6 bg-gray-50/50">
                            <div className="flex items-center mb-4">
                                <Activity className="h-5 w-5 text-[#9e46d3] mr-2" />
                                <h3 className="text-lg font-semibold text-gray-800">Análise de Saúde</h3>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-white rounded-lg p-4 border border-gray-200">
                                    <div className="flex items-center mb-3">
                                        <Instagram className="h-4 w-4 text-[#9e46d3] mr-2" />
                                        <span className="font-medium text-gray-800">@{currentProfile.username}</span>
                                    </div>
                                    <div className="grid grid-cols-3 gap-3 text-center">
                                        <div>
                                            <div className="text-lg font-bold text-gray-900">
                                                {currentProfile.followersCount >= 1000000
                                                    ? `${(currentProfile.followersCount / 1000000).toFixed(1)}M`
                                                    : currentProfile.followersCount >= 1000
                                                        ? `${(currentProfile.followersCount / 1000).toFixed(1)}K`
                                        ...
                                        <div>
                                            <div className="text-lg font-bold text-gray-900">
                                                {currentProfile.followsCount >= 1000
                                                    ? `${(currentProfile.followsCount / 1000).toFixed(1)}K`
                                                    : currentProfile.followsCount}
                                            </div>
                                ...
                                <ProfileHealthIndicator
                                    ...
                                    followsCount={currentProfile.followsCount}
                                ...
                                {/* Resumo das métricas */}
                                <div className="bg-white rounded-lg p-4 border border-gray-200">
                                    <h4 className="text-sm font-medium text-gray-800 mb-3">Critérios de Qualidade</h4>
                                    <div className="space-y-2 text-xs">
                                        <div className="flex justify-between">
                                            <span className="text-gray-600">Taxa Boa (1-3%)</span>
                                            <span className="text-blue-600">✓ Padrão da indústria</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-gray-600">Taxa Alta (3-6%)</span>
                                            <span className="text-green-600">✓ Público engajado</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-gray-600">Taxa Muito Alta (+6%)</span>
                                            <span className="text-purple-600">✓ Top 25% do Instagram</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
